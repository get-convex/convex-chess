    ...
    <main>
      {
        user?.profilePicUrl ?
          ...
          <div className="profileImage">{user?.name?.split(" ").map((w) => w.slice(0, 1).toUpperCase()).join("")}</div>
      }
      <div>{user?.name}</div>
      {
        ...
        <form onSubmit={handleSetProfilePic}>
          <input
            ...
            accept="image/*"
            ref={imageInput}
            onChange={event => setSelectedImage((event.target.files as any)[0])}
            className="ms-2 btn btn-primary"
            disabled={selectedImage !== null}
          />
          <input type="submit" value="Change Profile Pic" disabled={!selectedImage} />
        </form>
        : <div />
      }
    </main>
