...
Gateway's alpha Decisions endpoint. The rating appears below the move commentary;
select a move in the move list to see its rating. Existing moves are not backfilled.
This is an AI estimate, not a chess-engine evaluation.
...
The rating uses `getServiceToken("ai-gateway")` and `fetch`; no separate Jev API
key is needed. Jev's [score rubric](https://docs.typesafe.ai/primitives/score)
has ten levels from 0–9; the app adds one and rounds to one decimal place.
