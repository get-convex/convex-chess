...
type FilterJson = { $eq: [FilterJson, FilterJson] } | { $field: string } | JSONValue
...
class DatabaseFake {
    ...
    private _documents: Record<string, { tableName: string, document: GenericDocument }> = {}
    ...
    private _generateId<TableName extends string>(table: TableName): GenericId<TableName> {
    ...
    patch<TableName extends string>(id: GenericId<TableName>, value: Record<string, any>) {
      const result = this._documents[id];
      if (result === undefined) {
        throw new Error("Patch on non existent doc")
      }
      const { document, tableName } = result
      this._documents[id] = { document: {...document, ...value}, tableName }
      return
    }
    ...
    private _evaluateQuery(query: SerializedQuery): Array<GenericDocument> {
      ...
      switch (source.type) {
        ...
        case "IndexRange": {
          ...
          results = results.filter(v => source.range.every(f => evaluateRangeFilter(v, f)))
      ...
      const filters: Array<FilterJson> = query.operators.filter(o => (o as any).filter !== undefined).map(o => (o as any).filter)
      // @ts-ignore
      const limit: { limit: number } | null = query.operators.filter(o => (o as any).limit !== undefined)[0] ?? null
...
function evaluateFilter(document: GenericDocument, filter: any): Value | undefined {
  if (filter.$eq !== undefined) {
    return evaluateFilter(document, filter.$eq[0]) === evaluateFilter(document, filter.$eq[1])
  }
...
function evaluateRangeFilter(document: GenericDocument, expr: SerializedRangeExpression) {
...
function asyncSyscallImpl(op: string, jsonArgs: string): Promise<string> {
    ...
    switch (op) {
        ...
        case "1.0/shallowMerge": {
          const db = DatabaseFake.get();
          db.patch(args.id, jsonToConvex(args.value) as any);
          return Promise.resolve(JSON.stringify({}))
        }
        case "1.0/queryStreamNext": {
          const db = DatabaseFake.get();
          const { value, done } = db.queryNext(args.queryId)
          return Promise.resolve(JSON.stringify({ value: convexToJson(value as any), done }))
        }
        case "1.0/schedule": {
          const db = DatabaseFake.get();
          const id = db.insert("_scheduled_functions", {
            name: args.name,
            args: [args.args],
            scheduledTime: args.ts,
            state: { kind: "pending" }
          })
          return Promise.resolve(JSON.stringify(id))
        }
        default:
          throw new Error(`Async syscall ${op} not implemented`)
    }
}
...
export const setup = <Schema extends GenericSchema>(schema: SchemaDefinition<Schema, boolean> | null): TestConvex<SchemaDefinition<Schema, boolean>> => {
  DatabaseFake.initialize(schema)
  // @ts-ignore
  const previousConvex = global.Convex;
  // @ts-ignore
  global.Convex = {
    syscall: syscallImpl,
  ...
  const x: any = { 
    ...
    cleanup: ()  => {
      DatabaseFake.__instance = null
      // @ts-ignore
      global.Convex = previousConvex
      return
    }
  }
  return x
